...
if ($inputFileName != '' && $sheetName != '') {
    try {
        ...
        $testata['dataScadenzaPagamento'] = isset($rows[56][3]) ? Date::excelToDateTimeObject($rows[56][3])->format(
            'c'
        ) : '';
        ...
        foreach ($rows as $num => $row) {
            if ($num > 61) {
                ...
                $riga['riferimentoNumero2'] = isset($row[36]) ? (int)$row[36] : 0;
                $riga['riferimentoData2'] = isset($row[37]) ? Date::excelToDateTimeObject($row[37])->format(
                    'Y-m-d'
                ) : '';
                $righe[] = $riga;
            }
